...
export const BOOKS: Book[] = RAW.map(([name, chapters], i) => ({ id: i + 1, name, chapters }));
export const TOTAL_CHAPTERS = BOOKS.reduce((a, b) => a + b.chapters, 0); // 1189

/** 통독 순번 n(1..1189) → {book, chapter} */
export function chapterAt(n: number): { book: Book; chapter: number } {
  let acc = 0;
  for (const b of BOOKS) {
    if (n <= acc + b.chapters) return { book: b, chapter: n - acc };
    acc += b.chapters;
  }
